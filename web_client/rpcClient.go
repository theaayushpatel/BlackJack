...
func NewRpcClient(addr string) Rpc {
	...
	cl := pb.NewBlackJackClient(addr, grpcweb.WithDefaultCallOptions(grpcweb.ForceWebsocketTransport()))
	rcl.Client = cl
	return rcl
}
...
func (rpc *Rpc) StartEventListening(timeout time.Duration) (eventStream pb.BlackJack_EventListenClient, cancel context.CancelFunc, err error) {
