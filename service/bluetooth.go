...
import (
	...
	"github.com/theaayushpatel/BlackJack/service/bluetooth"
...
// BlackJack doesn't depend on late starting systemd services like DBus, bluetoothd etc.
// In order to assure that Bluetooth functionality is present, the BlackJack systemd service would have to depend
// on such "late starting" services. This again means the BlackJack daemon would load very late and functionality like
// USB gadgets wouldn't work till service the BlackJack systemd service gets started. Tests with current Kali releases have
// shown that BlackJack is up and reachable network after about 20 to 30 seconds.
// If the BlackJack systemd service is changed to depend on bluetooth.service this duration increases up to 2 minutes,
// which is NOT ACCEPTABLE.
// On the other hand, access to the bluez-stack and the hci device (if present) is already possible some seconds
// after the BlackJack service has started (doesn't take 2 minutes).
...
// happens in my tests) this means service startup of BlackJack increases by this duration (about 6 seconds in tests)
// On a system with a mis-configured bluetooth stack or with missing bluetooth hardware (Pi0 without WiFi/Bluetooth)
// this would mean BlackJack service startup consumes the full retryTime (shouldn't be the case, as we target a RPi0W
...
// BlackJack is meant to run headless, which has influence on Pairing mode. There's legacy pairing (outdated and insecure)
...
// As BlackJack could not display a passkey or request user input for a confirmation (assuming interactive access solutions
...
// and has to HAND OUT A ROUTER AND A DNS OPTION, POINTING TO THE IP OF BlackJack.
// Even if no upstream connection is provided, connecting to BlackJack is possible, if, and only if, these two DHCP options
...
//	bt.Controller.SetHighSpeed(true) // Enable high speed mode, yeah (without high speed, NAP connections don't work as intended)
	...
	err = bt.Controller.SetAlias("BlackJack")
	if err != nil {
		return
	}
