...
import (
	"context"
	"fmt"
	"github.com/theaayushpatel/BlackJack/service/pgpio"
	"periph.io/x/periph"
...
type GpioManager struct {
	availableGpioPins  []*pgpio.BlackJackPinIO
	availableGpioPinsMap  map[string]*pgpio.BlackJackPinIO
...
func NewGpioManager(rootSvc *Service) (res *GpioManager) {
	...
	gm.availableGpioPinsMap = make(map[string]*pgpio.BlackJackPinIO)
	gpios := gpioreg.All()
	for _, g := range gpios {
		if pinreg.IsConnected(g) {
			ppin := pgpio.NewBlackJackPinIO(g)
...
func (gm *GpioManager) GetAvailableGpios() (res []*pgpio.BlackJackPinIO, err error) {
	if gm.IsUsable {
