...
func (s *server) EventListen(eReq *pb.EventRequest, eStream pb.BlackJack_EventListenServer) (err error) {
...
func StartRpcServer(host string, port string) {
	...
	//Open TCP listener
	log.Printf("BlackJack RPC server listening on " + listen_address)
	...
	//Create gRPC Server
	s := grpc.NewServer()
	pb.RegisterBlackJackServer(s, &server{})
...
func StartRpcWebServer(host string, port string) {
	...
	//Open TCP listener
	log.Printf("BlackJack gRPC-web server listening on " + listen_address)
...
func (srv *server) StartRpcServerAndWeb(host string, gRPCPort string, webPort string, absWebRoot string) () {
	...
	//Create gRPC Server
	s := grpc.NewServer()
	pb.RegisterBlackJackServer(s, srv)
	...
	log.Printf("BlackJack gRPC server listening on " + listen_address_grpc)
	...
	log.Printf("BlackJack gRPC-web server listening on " + http_srv.Addr)
}
