...
import (
	...
	genl "github.com/theaayushpatel/BlackJack/mgenetlink"
	nl "github.com/theaayushpatel/BlackJack/mnetlink"
)

/*
Needs modified dwc2 kernel module, sending multicast generic netlink messages for genl family 'BlackJack' on multicast group 'dwc2'
...
var (
	EBlackJackFamilyMissing = errors.New("Couldn't find generic netlink family for BlackJack")
	EDwc2GrpMissing = errors.New("Couldn't find generic netlink mcast group for BlackJack dwc2")
	EDwc2GrpJoin = errors.New("Couldn't join generic netlink mcast group for BlackJack dwc2")
	EWrongFamily = errors.New("Message not from generic netlink family BlackJack")
)
...
func (d * Dwc2ConnectWatcher) evt_loop() {
	...
	for d.isRunning {
		fmt.Println("\nWaiting for messages from BlackJack kernel mods...\n")
...
func (d * Dwc2ConnectWatcher) Start() (err error){
	...
	// try to find GENL family for BlackJack
	d.fam,err = d.genl.GetFamily(fam_name)
	if err != nil {
		d.genl.Close()
		return EBlackJackFamilyMissing
	}
