...
var hidRunCmd = &cobra.Command{
	...
	Long:"Run script provided from standard input, commandline parameter or by path to script file on BlackJack",
	Run: cobraHidRun,
}
...
var hidJobCmd = &cobra.Command{
	...
	Long:"Run a background script provided from standard input, commandline parameter or by path to script file on BlackJack",
	Run: cobraHidJob,
}
...
//
// 1) `BlackJack_cli HID run` (no host flag, no additional args)
...
//
// 2) `BlackJack_cli HID run --host "some hostname"` (host flag, no additional args)
// Assume that the CLI runs on a host different from the gRPC server, read script from STDIN as no parameter is provided
// --> The script has to be run as remote script (content has to be transferred from CLI to gRPC server)
// after reading till EOF. Abort on timeout.
//
// 3) `BlackJack_cli HID run -c "Some HID script commands"`
// --> Same as 1) but content is read from `-c` instead of STDIN
//
// 4) `BlackJack_cli HID run --host "some hostname" -c "Some HID script commands"`
// --> Same as 2) but content is read from `-c` instead of STDIN
//
// 5) `BlackJack_cli HID run /some/path/to/file`
// Assume that the CLI runs on the same host as the server, assume that the given path refers to a file hosted on the
// gRPC server
// --> The script has to be run as local script (content is searched on BlackJack device by given path)
//
// 6) `BlackJack_cli HID run --host "some hostname" -r /some/path/to/file`
...
// --> The script has to be run as local script (content is searched on BlackJack device by given path)
//
// 7) `BlackJack_cli HID run --host "some hostname" /some/path/to/file` (no host flag, no additional args)
// Assume that the CLI runs on a host different from the gRPC server and the given path points to a script on the host
...
func parseHIDRunScriptCmd(cmd *cobra.Command, args []string) (serverScriptPath string, err error) {
	...
	*/
	...
	if transferNeeded {
		...
		if err != nil { return "",errors.New(fmt.Sprintf("Error transfering HIDScript content to BlackJack Server: %v", err))}
	}
