...
######
# This is a work in progress script for BlackJack A.L.O.A. based on @binkybear's built script for BlackJack
#
##########
...
basedir=`pwd`/rpi0w-nexmon-BlackJack-aloa-$1
TOPDIR=`pwd`

# Custom hostname variable
hostname=${2:-kali}
# Custom image name variable - MUST NOT include .img at the end.
imagename=${3:-kali-linux-$1-rpi0w-nexmon-BlackJack-aloa}
...
# dhcpcd5: REQUIRED (BlackJack A.L.O.A. currently wraps this binary if a DHCP client is needed)
# dnsmasq: REQUIRED (BlackJack A.L.O.A. currently wraps this binary if a DHCP server is needed, currently not used for DNS)
...
# move BlackJack in (change to release blob when ready)
git clone  -b 'v0.1.0-alpha2' --single-branch --depth 1  https://github.com/theaayushpatel/BlackJack "${basedir}"/kali-${architecture}/root/BlackJack
...
# dhcpcd is needed by BlackJack, but started on demand
...
# was explcitely enabled by BlackJack for this interface)
systemctl disable dhcpcd

# enable fake-hwclock (BlackJack is intended to reboot/loose power frequently without getting NTP access in between)
...
# Build nexmon firmware outside the build system, if we can (use repository with driver and firmware for BlackJack).
cd "${basedir}"
git clone https://github.com/mame82/nexmon_wifi_covert_channel.git -b BlackJack "${basedir}"/nexmon --depth 1

# Setup build
cd ${TOPDIR}
# Re4son kernel 4.14.80 with BlackJack patches (dwc2 and brcmfmac)
git clone --depth 1 https://github.com/Re4son/re4son-raspberrypi-linux -b rpi-4.14.80-re4son-BlackJack "${basedir}"/kali-${architecture}/usr/src/kernel
...
# Because of the BlackJack script, we set the hostname down here, instead of using the machine name.
# Set hostname
